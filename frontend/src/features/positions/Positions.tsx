...
const Positions = () => {
  const { data: positions, isLoading, error } = useQuery(['positions'], async () => {
  ...
  if (isLoading) {
    return <Box>Loading positions...</Box>;
  }

  if (error) {
    return <Box>Error loading positions</Box>;
  }
          ...
          <Table variant="simple">
            ...
            <Tbody>
              {positions.map((position: any) => {
                const change = ((position.last_price - position.average_price) / position.average_price) * 100;
